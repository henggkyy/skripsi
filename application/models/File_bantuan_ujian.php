...
class File_bantuan_ujian extends CI_Model{
	//Method untuk mendapatkan individual item berdasarkan path file
	function getIndividualItemByPath($path, $item){
		$this->db->select($item);
		$this->db->where('PATH_FILE', $path);
	...
	//Method untuk mendapatkan individual item berdasarkan id file bantuan
	...
	//Method untuk menolak file bantuan ujian
	function rejectFileBantuan($id, $data){
		$this->db->where('ID', $id);
		$res = $this->db->update('file_bantuan_ujian', $data);
		if($res){
			return true;
		}
		else{
	...
	//Method untuk menerima file bantuan ujian
	function acceptFileBantuan($id, $data){
		$this->db->where('ID', $id);
		$res = $this->db->update('file_bantuan_ujian', $data);
		if($res){
			return true;
	...
	function getFileBantuan($id_matkul){
		$this->db->select('file_bantuan_ujian.ID as ID, file_bantuan_ujian.PATH_FILE as PATH_FILE, file_bantuan_ujian.NAMA_FILE_USER as NAMA_KETERANGAN, file_bantuan_ujian.TIPE_UJIAN as TIPE_UJIAN, file_bantuan_ujian.LAST_UPDATE as LAST_UPDATE, file_bantuan_ujian.STATUS as STATUS, users.NAMA as USER_UPLOAD');
		$this->db->from('file_bantuan_ujian');
